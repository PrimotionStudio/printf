 ...
 */
int mq_handle_print(const char *mq_fmt, int *mq_ind,
		...
		int mq_flags, int mq_width, int mq_precision, int mq_size)
{
	...
	for (iter = 0; fmt_types[iter].mq_fmt != '\0'; iter++)
	{
		if (mq_fmt[*mq_ind] == fmt_types[iter].mq_fmt)
		{
			return (fmt_types[iter].mq_fn(mq_list, mq_buffer,
					mq_flags, mq_width, mq_precision, mq_size));
		}
	}
	if (fmt_types[iter].mq_fmt == '\0')
	{
		if (mq_fmt[*mq_ind] == '\0')
			return (-1);
		...
		{
			--(*mq_ind);
			while (mq_fmt[*mq_ind] != ' ' && mq_fmt[*mq_ind] != '%')
				--(*mq_ind);
			if (mq_fmt[*mq_ind] == ' ')
				--(*mq_ind);
			return (1);
		}
		mq_unknown_len += write(1, &mq_fmt[*mq_ind], 1);
